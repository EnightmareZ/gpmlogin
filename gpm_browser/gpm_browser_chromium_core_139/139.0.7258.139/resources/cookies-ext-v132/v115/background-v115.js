...
function send_data_to_socket(json_data){
    ...
    json_data.data.profile_id = _currentProfileId; // Global version
    
    if(_socketConnected){
        _socket.send(JSON.stringify(json_data));
    }else{
        console.error('Socket is not connect');
    }
}
...
function process_socket_response(response){
    ...
    let command = dataFromServer.end_point ?? dataFromServer.command;
    ...
    switch(command){
        ...
        case 'close':
            ...
            break;
        case 'error':
            console.error(JSON.stringify(dataFromServer));
            break;
    }
}
...
async function connect_socket_server() {
    if ("WebSocket" in window) {
        ...
        if(profile_id) {
            ...
            config.profile_path     = user_data_dir;
            config.file_cookie_save = `${user_data_dir}\\Default\\GPMSoft\\Exporter\\ExportCookies.json`; // From v137
            config.socket_connected = false;

            if(!config.websocket_server)
            {
                console.error('Not found websocket_server!');
                return;
            }
            ...
            console.error('Can not get profile id');
        }
    } else {
