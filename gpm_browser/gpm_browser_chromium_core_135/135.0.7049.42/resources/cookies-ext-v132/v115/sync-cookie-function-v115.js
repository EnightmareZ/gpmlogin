...
const END_POINT_CONVERT_COOKIE_RESULT = 'cookie-v115-convert-result';
    ...
    return new Promise((resolve, reject) => {
        chrome.cookies.set(cookie, () => {
            if (chrome.runtime.lastError) {
                log_error('Cannot set cookie.' + chrome.runtime.lastError.message);
...
function post_data_to_http_server_on_tool(url, body){
    ...
    .catch((error) => log_error(`Failed post to ${url}: ${error.message}`));
}
...
async function sendCookieToToolOnDemand() {
    ...
    chrome.cookies.getAll({}, (cookies) => {
        ...
        let dataToSend = JSON.stringify({
            ...
            gpm_profile_id: config.gpm_profile_id,
            file_cookie_save: config.file_cookie_save,
            data: cookiesBody
        });
        fetch(`${config.url_server}/export-cookies`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        .then(() => {
            log_info('sent cookies to server');
        })
        .catch((error) => log_error(`Failed to sent cookie: ${error.message}`));
    });
}
...
function restoreCookieFromFile(gpm_data) {
    ...
    chrome.cookies.getAll({}, async (cookies) => {
        ...
        let file_cookie_save    = `${user_data_dir}\\Default\\GPMBrowserExtenions\\cookies-ext-new\\gpm_restore_cookie.json`;
        ...
        .catch((error) => {
            log_error(`Failed post to ${url}: ${error.message}`);
            post_data_to_http_server_on_tool(`${sync_config.url_server}/${END_POINT_CONVERT_COOKIE_RESULT}`, {
                success: false,
                gpm_profile_id: profile_id,
                file_cookie_save: file_cookie_save,
    ...
    try {
        ...
        if(cookie_restore_data_json) {
            ...
            if (cookie_restore_data_json.id == null || idRestored.indexOf(cookie_restore_data_json.id) == -1) {
                log_info(`restoring v115/gpm_restore_cookie.json (id=${cookie_restore_data_json.id})...`);
                restoreCookieFromFile(cookie_restore_data_json);
            } else {
                log_info(`Skip v115/gpm_restore_cookie.json (id=${cookie_restore_data_json.id})`);
            }
        }
    }
    catch {
        log_error('error when restore v115/gpm_restore_cookie.json');
    }
}
restore_cookie_data();
